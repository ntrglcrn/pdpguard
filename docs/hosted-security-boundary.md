...
| Screenshots       | PNG files use random UUIDs, mode `0600`, local `.runtime` storage and opportunistic 24-hour cleanup. The GET route validates UUID shape but has no authentication or ownership check.                                                  | Durable artifacts are bound to an audit run and tenant, retrieved only after object-level authorization, and governed by enforced retention/deletion. Local filesystem paths are not part of the hosted contract. |
| Audit data        | Results are returned to the caller but no audit history is persisted. Browser state is discarded when the browser closes.                                                                                                              | Persist only the tenant-owned run record, deterministic findings, bounded evidence, metadata and artifact references required by the product.                                                                     |
...
| Tenant boundary   | None. The local MVP has no accounts, workspaces, stores or object authorization.                                                                                                                                                       | Every operation resolves and authorizes the full Workspace → Store → Audit Run → Finding / Screenshot ownership chain.                                                                                            |
...
There are currently no tests for tenant authorization, worker isolation,
infrastructure egress, durable cancellation/retry or hosted artifact access
because those capabilities do not yet exist.
